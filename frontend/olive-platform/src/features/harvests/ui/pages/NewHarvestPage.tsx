...
export default function NewHarvestPage() {
  ...
  const getValidationErrors = useCallback((): Record<string, string> => {
    ...
    if (form.plannedTrees <= 0) validationErrors.harvestedTrees = "Le nombre d'arbres doit être supérieur à 0."
    ...
    <div className="feature-page">
      <div className="page-header">
        <div className="page-header-content">
          ...
          <p className="page-description">Créer une nouvelle récolte et renseigner les informations associées.</p>
        </div>
      </div>
      ...
      <div className="filters">
        ...
        <div className="filters-content">
          <div className="filter-item">
            ...
            {errors.plotId && <span className="field-error">{errors.plotId}</span>}
          </div>
          ...
          {form.plotId > 0 && form.varietyId > 0 && (
            <div style={{ gridColumn: '1 / -1', marginTop: '10px', paddingTop: '20px', borderTop: '1px solid #eee' }}>
              <div className="available-trees-info">
                {availableTreesLoading && <span>Calcul du nombre d'arbres disponibles...</span>}
                {!availableTreesLoading && availableTrees !== null && (
                  <strong>{availableTrees.toLocaleString('fr-FR')} arbres disponibles pour la récolte</strong>
                )}
                {availableTreesError && <span className="field-error">{availableTreesError}</span>}
              </div>
            </div>
          )}
      ...
      <div className="filters-footer">
        <Button variant="secondary" onClick={handleCancel} disabled={saving}>Annuler</Button>
        <Button variant="primary" onClick={handleSubmit} disabled={saving || !isFormValid}>
