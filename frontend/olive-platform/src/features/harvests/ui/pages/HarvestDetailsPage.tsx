...
export default function HarvestDetailsPage() {
  ...
  const [completeForm, setCompleteForm] =
    useState<CompleteHarvestForm>(emptyCompleteForm());
  ...
  const updateForm = useCallback(
    <K extends keyof HarvestForm>(field: K, value: HarvestForm[K]) => {
      setForm((previous) =>
  ...
  const handleOpenCompleteDrawer = useCallback(() => {
    if (!harvest) return;
    setCompleteForm(
      emptyCompleteForm(harvest.quantityKg ?? 0, harvest.harvestedTrees ?? 0),
    );
    setErrors({});
  ...
  const handleAddStock = useCallback(() => {
    setCompleteForm((previous) => ({
      ...previous,
      stocks: [...previous.stocks, { quantityKg: 0 }],
    }));
  }, []);
  ...
  const validateCompleteForm = useCallback((): Record<string, string> => {
    ...
    if (completeForm.stocks.length === 0)
      validationErrors.stocks = "Au moins un stock doit être renseigné.";

    completeForm.stocks.forEach((stock, index) => {
      if (stock.quantityKg <= 0)
        validationErrors[`stock_${index}_quantityKg`] =
    ...
    if (
      ...
      Math.abs(totalStocksQuantity - completeForm.quantityKg) > 0.001
    ) {
  ...
  const handleCancelHarvest = useCallback(async () => {
    if (!harvest) return;
    if (!window.confirm("Êtes-vous sûr de vouloir abandonner cette récolte ?"))
      return;
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
          ...
          <div className="filter-item">
            <TextInput
              ...
              onChange={(event) =>
                updateCompleteForm("completionDate", event.target.value)
              }
              disabled={saving}
            />
            {errors.completionDate && (
              <span className="field-error">{errors.completionDate}</span>
            )}
          </div>
          ...
          <div className="filter-item">
            <TextInput
              ...
              onChange={(event) =>
                updateCompleteForm("harvestedTrees", Number(event.target.value))
              }
              disabled={saving}
            />
            ...
            {harvest.plannedTrees != null && (
              <small>
                {harvest.plannedTrees.toLocaleString("fr-FR")} arbres planifiés
              </small>
            )}
          </div>
          ...
          <div className="filter-item">
            <TextInput
              ...
              onChange={(event) =>
                updateCompleteForm("quantityKg", Number(event.target.value))
              }
              disabled={saving}
            />
            {errors.quantityKg && (
              <span className="field-error">{errors.quantityKg}</span>
            )}
          </div>
          ...
          >
            ...
            >
              <div>
                ...
                <div style={{ marginTop: "4px" }}>
                  Répartissez la quantité récoltée entre les stocks.
                </div>
              </div>
              <Button
                variant="secondary"
            ...
            {errors.stocks && (
              <span className="field-error">{errors.stocks}</span>
            )}
            ...
            >
              ...
              >
                <span>
                  {remainingQuantity >= 0 ? "Reste à répartir" : "Dépassement"}
                </span>
