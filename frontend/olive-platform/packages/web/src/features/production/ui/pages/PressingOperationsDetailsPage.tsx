...
} from "@olive-platform/core/features/production/stores/PressingOperationDetailsStore";

import {
  usePressingOperationInputsStore,
} from "@olive-platform/core/features/production/stores/PressingOperationInputsStore";
...
import PressingOperationTabs, {
  type PressingOperationTab,
} from "../components/PressingOperationTabs";

import type { UpdatePressingOperationParams } from "@olive-platform/core/features/production/domain/params/UpdatePressingOperationParams";
...
export default function PressingOperationDetailsPage() {
  ...
  const [
    activeTab,
    setActiveTab,
  ] = useState<PressingOperationTab>("general");
  ...
  const oliveQuantityKg =
    operation?.oliveQuantityKg ?? 0;
  ...
  const yieldPercentage = (() => {
    ...
    return (
      (Number(operation.oilQuantityLiters) /
        oliveQuantityKg) *
      100
    ).toFixed(2);
  })();
  ...
  const isInProgress =
    operation?.status === ProductionStatus.InProgress;
